import { chaseMarkup, playChase } from "./chase.js";

// Decorative rewards never change an answer, score, or test timer.
// Warm the celebration sheet; the vector chase needs no image downloads.
const spriteSheets = ["bear-dance-60"];
...
function play(scene, duration) {
  // Replay decorations; wait for the image only when this scene uses it.
  ...
  (scene.querySelector(".chase-pack") ? Promise.resolve() : spriteReady).then(
    () => {
